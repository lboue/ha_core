...
START_STREAM_RESPONSE = {
    "webrtc_session_id": MATTER_SESSION_ID,
    "mode": "provide_offer",
    "video": {"stream_id": 10, "codec": "H264", "reused": False},
    "audio": None,
}
CURRENT_SESSION = {
    "0": MATTER_SESSION_ID,
...
async def _start_session(
    ...
    matter_client.send_command.return_value = START_STREAM_RESPONSE
...
def _stop_stream_calls(matter_client: MagicMock) -> list[Any]:
    """Return the camera_stop_stream calls sent to the server."""
    return [
        call_args
        for call_args in matter_client.send_command.call_args_list
        if call_args.args[0] == "camera_stop_stream"
    ]
...
async def test_camera_webrtc(
    ...
    # The server is asked to start a live view stream with the frontend offer.
    assert matter_client.send_command.call_args == call(
        "camera_start_stream",
        require_schema=14,
    ...
    matter_client.send_webrtc_provider_command.assert_not_called()
    ...
    # Closing the session asks the server to stop the stream.
    await client.send_json_auto_id(
        {
...
async def test_camera_webrtc_candidate_before_answer(
    ...
    async def blocked_offer(*args: Any, **kwargs: Any) -> dict[str, Any]:
        await release.wait()
        return START_STREAM_RESPONSE

    matter_client.send_command.side_effect = blocked_offer
    client = await hass_ws_client(hass)
...
async def test_camera_webrtc_answer_races_offer(
    ...
    async def racing_offer(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    matter_client.send_command.side_effect = racing_offer
    client = await hass_ws_client(hass)
...
@pytest.mark.parametrize(
    "error",
    [
        pytest.param(MatterError("boom"), id="matter_error"),
        pytest.param(ServerVersionTooOld("too old"), id="server_too_old"),
    ],
)
async def test_camera_webrtc_offer_error(
    ...
    matter_client.send_command.side_effect = error
    client = await hass_ws_client(hass)
...
async def test_camera_end_event(
    ...
    """Test that an 'end' event cleans up so close does not stop the stream."""
    client = await hass_ws_client(hass)
    ...
    assert response["success"] is True
    await hass.async_block_till_done()
    assert _stop_stream_calls(matter_client) == []
...
async def test_camera_image(
    ...
    """Test that the camera returns a snapshot taken by the server."""
...
@pytest.mark.parametrize(
    "error",
    [
        pytest.param(MatterError("no snapshot"), id="matter_error"),
        pytest.param(ServerVersionTooOld("too old"), id="server_too_old"),
    ],
)
@pytest.mark.usefixtures("matter_node")
async def test_camera_image_placeholder(
    hass: HomeAssistant,
    matter_client: MagicMock,
    error: Exception,
) -> None:
    """Test that the placeholder is returned when no snapshot can be taken."""
    matter_client.send_command.side_effect = error
    image = await async_get_image(hass, ENTITY_ID)
